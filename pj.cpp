...
{
public:
    LineFollowingRobot(const std::string &robot_name, bool go_left)
        : Node(robot_name), go_left_(go_left)
    {
        // 카메라 토픽 구독
...
private:
    void image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        if (!contours.empty())
        {
            ...
            // 분기점에서 왼쪽 또는 오른쪽으로 회전
            if (contours.size() > 1) // 분기점 감지 (간단한 예시)
            {
                if (go_left_)
                {
                    // 왼쪽 로봇: 왼쪽으로 회전
...
int main(int argc, char *argv[])
{
    ...
    auto left_robot = std::make_shared<LineFollowingRobot>("left_robot", true);
    auto right_robot = std::make_shared<LineFollowingRobot>("right_robot", false);

    rclcpp::spin(left_robot);
    rclcpp::spin(right_robot);
